        ...
        {dmarcHistory && dmarcHistory.length > 0 ? dmarcHistory.slice(0,3).map((h,i) => (
          <div key={i} onClick={() => setPage('dmarc')}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:10.5, color:'#9ca3af', marginTop:2 }}>{h.dmarc_policy?`p=${h.dmarc_policy}`:'No policy'} · {h.score>=80?'Strong':h.score>=50?'Moderate':'Weak'}</div>
            </div>
            <div style={{ display:'flex', gap:3, flexShrink:0 }}>
              {[h.dmarc_status,h.spf_status,h.dkim_status].map((st,j) => (
                <div key={j} style={{ width:8, height:8, borderRadius:2.5, background:st==='great'||st==='good'?'#10b981':st==='warn'?'#f59e0b':'#dc2626' }}/>
              ))}
            </div>
          </div>
        )) : (
