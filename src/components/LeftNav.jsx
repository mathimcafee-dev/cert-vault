export default function LeftNav({ page, setPage, csrCount, inventoryAlerts }) {
  const nav = (id) => {
    if (id.startsWith('dmarc')) setPage('dmarc')
    else if (id.startsWith('dns')) setPage('dns')
    else setPage(id)
  }

  const SI = ({ id, emoji, label, count, countRed, bg, active }) => {
    const on = active !== undefined ? active : page === id
    return (
      <div onClick={() => nav(id)}
  ...
  return (
    <div style={{ width:224, background:'#fff', borderRight:'1.5px solid #cffafe', display:'flex', flexDirection:'column', flexShrink:0, overflowY:'auto' }}>
      ...
      <div style={{ padding:'0 12px 4px' }}>
        ...
        <SI id="dmarc"          emoji="📧" label="DMARC Check"    bg="#eff8ff" active={page==='dmarc'}/>
        <SI id="dmarc-bulk"     emoji="📨" label="Bulk DMARC"     bg="#f0fdf4"/>
        <SI id="dmarc-spf"      emoji="📮" label="SPF Generator"  bg="#fefce8"/>
        <SI id="dmarc-dkim"     emoji="🛡" label="DKIM Generator" bg="#fdf4ff"/>
        <SI id="dmarc-bimi"     emoji="🏷" label="BIMI Generator" bg="#fafafe"/>
        <SI id="dmarc-phishing" emoji="🎣" label="Phishing Check" bg="#fff5f4"/>
      </div>
      <Div/>
      <div style={{ padding:'0 12px 4px' }}>
        <Lbl>DNS Toolkit</Lbl>
        <SI id="dns"          emoji="🔍" label="DNS Lookup"     bg="#f0f9ff" active={page==='dns'}/>
        <SI id="dns-health"   emoji="🏥" label="Domain Health"  bg="#f0fdf4"/>
        <SI id="dns-prop"     emoji="🌍" label="Propagation"    bg="#fefce8"/>
        <SI id="dns-bl"       emoji="🚫" label="Blacklist Check"bg="#fff5f4"/>
      ...
      <div style={{ padding:'0 12px 4px' }}>
        <Lbl>Coming Soon</Lbl>
        {[['ssl-checker','🔒','SSL Checker'],['cert-labs','🧪','Certificate Labs']].map(([id,emoji,label]) => (
          <div key={id} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 8px', borderRadius:9, fontSize:12.5, fontWeight:500, color:'#9ca3af', opacity:.6, marginBottom:2 }}>
            <div style={{ width:26, height:26, borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, flexShrink:0, background:'#f5f3f0' }}>{emoji}</div>
