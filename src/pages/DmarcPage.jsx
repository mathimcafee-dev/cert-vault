...
const TABS = [
  ...
  { id:'bimi-gen',  label:'BIMI Generator'       },
  { id:'headers',   label:'Email Headers'        },
  { id:'phishing',  label:'Phishing Checker'     },
]

export default function DmarcPage({ user, initialTab, onCheck }) {
  ...
  async function handleSingleCheck(e) {
    ...
    try {
      ...
      setResult(r)
      const histEntry = { id:Date.now(), domain:d, score:r.score, dmarc_status:r.dmarc.status, spf_status:r.spf.status, dkim_status:r.dkim.status, dmarc_policy:r.dmarc.policy, checked_at:r.checkedAt, raw_json:{dmarc:r.dmarc,spf:r.spf,dkim:r.dkim} }
      setHistory(prev => [histEntry, ...prev].slice(0,30))
      if (onCheck) onCheck(histEntry)
  ...
  async function runBulk() {
    ...
    setHistory(prev => [...results.map((r,idx) => ({ id:Date.now()+idx, domain:r.domain, score:r.score||0, dmarc_status:r.dmarc?.status, spf_status:r.spf?.status, dkim_status:r.dkim?.status, checked_at:r.checkedAt, raw_json:{dmarc:r.dmarc,spf:r.spf,dkim:r.dkim} })), ...prev].slice(0,100))
  }
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:'#f5f6fa'}}>
      ...
      <div style={{padding:'14px 28px',background:'#fff',borderBottom:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',gap:6}}>
          ...
          {history.length>0&&<button onClick={clearHistory} style={{padding:'7px 14px',background:'#fef2f2',color:'#dc2626',border:'1px solid #fca5a5',borderRadius:8,fontSize:12.5,fontWeight:600,cursor:'pointer'}}>Clear All</button>}
        </div>
      </div>
        ...
        {tab==='bulk' && (
          <>
            <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:'18px 20px',marginBottom:20}}>
              ...
              <div style={{marginTop:7,fontSize:11.5,color:'#9ca3af'}}>Domain column detected automatically. All domains checked in parallel. Results stored in session only.</div>
            </div>
            ...
            {!bulkDomains.length&&!bulkRunning&&!bulkDone&&<EmptyState icon="📋" title="Upload a CSV to bulk check domains" desc="Domain column is detected automatically. All domains checked in parallel. Results stored in session only."/>}
          </>
        )}
...
function BimiGenerator() {
  ...
  const isValid = domain.trim() && logoUrl.trim()
  const record  = isValid
    ? `v=BIMI1; l=${logoUrl.trim()}${vmcUrl.trim() ? `; a=${vmcUrl.trim()}` : ''}`
    : ''
  const dnsName = domain.trim() ? `${selector}._domainkey.${domain.trim()}`.replace(`${selector}._domainkey.`,`${selector}._bimi.`) : ''
  const fullDnsName = domain.trim() ? `${selector}._bimi.${domain.trim()}` : ''
  ...
  const SVG_REQUIREMENTS = [
    'Square aspect ratio (1:1)',
    'SVG format only — no PNG, JPG or GIF',
    'Hosted at a public HTTPS URL',
    'Must be a Tiny PS (Tiny Portable/Secure) SVG',
    'File size recommended under 32KB',
    'No scripts, no external references',
    'Brand logo must fill the entire canvas',
  ]

  const MAIL_CLIENTS = [
    { name:'Gmail',             support:'✓ VMC/CMC required',   color:'#059669' },
    { name:'Apple Mail',        support:'✓ VMC required',        color:'#059669' },
    { name:'Yahoo Mail',        support:'✓ No VMC needed',       color:'#059669' },
    { name:'Fastmail',          support:'✓ No VMC needed',       color:'#059669' },
    { name:'AOL Mail',          support:'✓ No VMC needed',       color:'#059669' },
    { name:'Proton Mail',       support:'⚠ Partial support',     color:'#d97706' },
    { name:'Zoho Mail',         support:'⚠ Partial support',     color:'#d97706' },
    { name:'Outlook / M365',    support:'✕ Not supported',       color:'#dc2626' },
  ]
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16,maxWidth:800}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 140px',gap:10,marginBottom:12}}>
          ...
          <div>
            ...
            <input value={selector} onChange={e=>setSelector(e.target.value)} placeholder="default"
              style={{width:'100%',padding:'10px 14px',border:'1px solid #e5e7eb',borderRadius:9,fontSize:13,color:'#111827',outline:'none'}}
              onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='#e5e7eb'}/>
          </div>
        </div>
        <div style={{marginBottom:12}}>
          <label style={{display:'block',fontSize:11,fontWeight:700,color:'#6b7280',marginBottom:6,textTransform:'uppercase',letterSpacing:'0.05em'}}>Logo SVG URL <span style={{color:'#dc2626'}}>*</span></label>
          <input value={logoUrl} onChange={e=>setLogoUrl(e.target.value)} placeholder="https://yoursite.com/logo.svg"
            style={{width:'100%',padding:'10px 14px',border:'1px solid #e5e7eb',borderRadius:9,fontSize:13,color:'#111827',outline:'none'}}
            onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='#e5e7eb'}/>
          <div style={{fontSize:11.5,color:'#9ca3af',marginTop:5}}>Must be a publicly accessible HTTPS URL pointing to your SVG logo</div>
        </div>
        <div>
          <label style={{display:'block',fontSize:11,fontWeight:700,color:'#6b7280',marginBottom:6,textTransform:'uppercase',letterSpacing:'0.05em'}}>VMC/CMC Certificate URL <span style={{color:'#9ca3af',fontWeight:400,fontSize:10}}>(optional — required for Gmail & Apple Mail)</span></label>
          <input value={vmcUrl} onChange={e=>setVmcUrl(e.target.value)} placeholder="https://yoursite.com/vmc.pem"
            style={{width:'100%',padding:'10px 14px',border:'1px solid #e5e7eb',borderRadius:9,fontSize:13,color:'#111827',outline:'none'}}
            onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='#e5e7eb'}/>
        </div>
      </div>
      ...
      {isValid && (
        <div style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:12,padding:'18px 20px'}}>
          <div style={{fontSize:12,fontWeight:700,color:'#059669',marginBottom:4}}>Generated BIMI Record</div>
          <div style={{fontSize:11.5,color:'#059669',marginBottom:10}}>Add this as a TXT record at: <code style={{fontFamily:'monospace',fontWeight:600}}>{fullDnsName}</code></div>
          <div style={{display:'flex',gap:8,alignItems:'flex-start',marginBottom:12}}>
            <code style={{flex:1,background:'#fff',border:'1px solid #bbf7d0',borderRadius:8,padding:'12px 14px',fontSize:12.5,color:'#111827',fontFamily:'monospace',wordBreak:'break-all',display:'block'}}>{record}</code>
            <button onClick={()=>copy(record,'record')} style={{padding:'10px 16px',background:'#059669',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{copied==='record'?'Copied!':'Copy Record'}</button>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            <div style={{background:'#fff',border:'1px solid #bbf7d0',borderRadius:8,padding:'10px 14px'}}>
              <div style={{fontSize:10.5,fontWeight:700,color:'#6b7280',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4}}>DNS Record Type</div>
              <div style={{fontSize:13,fontWeight:600,color:'#111827'}}>TXT</div>
            </div>
            <div style={{background:'#fff',border:'1px solid #bbf7d0',borderRadius:8,padding:'10px 14px'}}>
              <div style={{fontSize:10.5,fontWeight:700,color:'#6b7280',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4}}>DNS Host / Name</div>
              <code style={{fontSize:12,fontWeight:600,color:'#111827',fontFamily:'monospace'}}>{fullDnsName}</code>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Prerequisites */}
      <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:'18px 20px'}}>
        <div style={{fontSize:13,fontWeight:700,color:'#374151',marginBottom:14}}>Prerequisites — Before publishing BIMI</div>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[
            {ok:true,  text:'DMARC policy must be p=quarantine or p=reject (not p=none)'},
            {ok:true,  text:'SPF record must be published and passing'},
            {ok:true,  text:'DKIM must be configured and signing emails'},
            {ok:false, text:'VMC/CMC required for Gmail and Apple Mail display — Yahoo/Fastmail work without it. Outlook does not support BIMI at all.'},
          ].map((item,i)=>(
            <div key={i} style={{display:'flex',gap:10,alignItems:'flex-start',padding:'10px 12px',background:item.ok?'#f0fdf4':'#fffbeb',border:`1px solid ${item.ok?'#bbf7d0':'#fde68a'}`,borderRadius:8}}>
              <span style={{fontSize:13,fontWeight:700,color:item.ok?'#059669':'#d97706',flexShrink:0,marginTop:1}}>{item.ok?'✓':'!'}</span>
              <span style={{fontSize:12.5,color:'#374151'}}>{item.text}</span>
            </div>
          ))}
        </div>
      </div>

      {/* SVG requirements + email client support side by side */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:'16px 18px'}}>
          <div style={{fontSize:13,fontWeight:700,color:'#374151',marginBottom:12}}>SVG Logo Requirements</div>
          {SVG_REQUIREMENTS.map((r,i)=>(
            <div key={i} style={{display:'flex',gap:8,alignItems:'flex-start',marginBottom:8}}>
              <div style={{width:20,height:20,borderRadius:6,background:'#eef2ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,color:'#4f46e5',flexShrink:0,marginTop:1}}>{i+1}</div>
              <span style={{fontSize:12.5,color:'#374151',lineHeight:1.5}}>{r}</span>
            </div>
          ))}
        </div>
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:'16px 18px'}}>
          <div style={{fontSize:13,fontWeight:700,color:'#374151',marginBottom:12}}>Email Client Support</div>
          {MAIL_CLIENTS.map((c,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 0',borderBottom:i<MAIL_CLIENTS.length-1?'1px solid #f5f5f5':'none'}}>
              <span style={{fontSize:13,fontWeight:500,color:'#374151'}}>{c.name}</span>
              <span style={{fontSize:11.5,fontWeight:700,color:c.color}}>{c.support}</span>
            </div>
          ))}
          <div style={{marginTop:12,padding:'10px 12px',background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:8}}>
            <div style={{fontSize:11.5,color:'#dc2626',lineHeight:1.6,fontWeight:500}}>Outlook and Microsoft 365 do not support BIMI as of 2025. Logos will not display for Outlook recipients regardless of VMC.</div>
          </div>
        </div>
      </div>
      ...
      {/* How to verify */}
      <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:'16px 18px'}}>
        <div style={{fontSize:13,fontWeight:700,color:'#374151',marginBottom:12}}>How to verify after publishing</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:10}}>
          {[
            {step:'1',title:'Publish DNS record',desc:'Add the TXT record at the BIMI address shown above'},
            {step:'2',title:'Wait for propagation',desc:'Allow 24–48 hours for DNS to propagate globally'},
            {step:'3',title:'Use BIMI Checker tab',desc:'Switch to the BIMI Checker tab and verify your domain'},
          ].map(s=>(
            <div key={s.step} style={{padding:'12px 14px',background:'#f8fafc',border:'1px solid #e5e7eb',borderRadius:9}}>
              <div style={{width:24,height:24,borderRadius:7,background:'#4f46e5',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#fff',marginBottom:8}}>{s.step}</div>
              <div style={{fontSize:12.5,fontWeight:700,color:'#111827',marginBottom:4}}>{s.title}</div>
              <div style={{fontSize:11.5,color:'#6b7280',lineHeight:1.5}}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
