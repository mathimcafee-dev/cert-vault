const TOOLS = [
  {
    ...
    features: ['Full certificate chain validation','TLS 1.2 / 1.3 protocol detection','Cipher suite analysis','HSTS and security header check','Mixed content detection','Real-time results with color coding'],
    eta: 'Q2 2026',
  },
  {
    id: 'cert-labs',
    ...
    features: ['PEM / DER / PFX / P12 decoder','Certificate chain builder','Format converter (all formats)','Expiry and validity checker','Subject Alternative Names viewer','Fingerprint and signature analysis'],
    eta: 'Q2 2026',
  },
]
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'auto', background:'#f5f6fa' }}>
      ...
      <div style={{ padding:'32px 28px', maxWidth:860, width:'100%', margin:'0 auto' }}>
        <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:16, padding:'36px', marginBottom:24, textAlign:'center', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'10px 24px', background:tool.color, color:'#fff', borderRadius:10, fontSize:13, fontWeight:700, opacity:0.7, cursor:'not-allowed' }}>🔒 Not yet available</div>
        </div>
        <div style={{ marginBottom:24 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>What will be included</div>
        ...
        {TOOLS.filter(t => t.id !== tool.id).length > 0 && (
          <div>
            <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:12 }}>Also coming soon</div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2, minmax(0,1fr))', gap:10 }}>
              {TOOLS.filter(t => t.id !== tool.id).map(t => (
                <div key={t.id} style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:10, padding:'14px 16px', display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div>
                    <div style={{ fontSize:12.5, fontWeight:600, color:'#374151' }}>{t.label}</div>
