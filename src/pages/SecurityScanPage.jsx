...
// SSL CHECK — Vercel proxy (server-side, no CORS) with crt.sh fallback
async function checkSSLLabs(domain) {
  const result = {
    grade: null, subject: null, issuer: null, expires: null,
    daysLeft: null, keyAlg: null, keySize: null, protocols: [],
    vulns: [], hsts: false, hstsAge: null, ipAddress: null,
    fromCache: false, source: null, error: null, crtEntries: [],
  }

  // Step 1 — Try Vercel proxy for SSL Labs (server-side, no CORS restriction)
  try {
    ...
    result.error = 'Proxy unavailable: ' + e.message
  }

  // Step 2 — crt.sh Certificate Transparency (CORS-friendly, always works)
  try {
    const r = await fetch(
    ...
    if (r.ok) {
      ...
      if (Array.isArray(raw) && raw.length > 0) {
        ...
        const valid  = sorted.filter(c => new Date(c.not_after) > now && (
          c.common_name === domain || c.common_name === `*.${domain}` ||
          (c.name_value || '').split('\n').some(n => n === domain || n === `*.${domain}`)
        ))
        const validFallback = sorted.filter(c => new Date(c.not_after) > now)
        const pool = valid.length > 0 ? valid : validFallback

        result.crtEntries = sorted.slice(0, 8).map(c => {
          const exp = new Date(c.not_after)
          return {
            issuer:  c.issuer_name?.match(/O=([^,/]+)/)?.[1]?.trim() || c.issuer_name?.match(/CN=([^,/]+)/)?.[1]?.trim() || '—',
            subject: c.common_name || '—',
            from:    new Date(c.not_before).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}),
            to:      exp.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}),
        ...
        if (pool.length > 0) {
          ...
          result.issuer   = latest.issuer_name?.match(/O=([^,/]+)/)?.[1]?.trim() || latest.issuer_name?.match(/CN=([^,/]+)/)?.[1]?.trim() || '—'
          ...
          result.source = 'Certificate Transparency (crt.sh)'
          result.error  = 'No valid SSL certificate found in CT logs for this domain'
        }
      }
    }
  } catch (e) {
    if (!result.error) result.error = 'CT log lookup failed: ' + e.message
  }
