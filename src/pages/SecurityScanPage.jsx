...
// SSL CHECK — calls our Vercel proxy which handles SSL Labs + crt.sh server-side
async function checkSSLLabs(domain) {
  ...
  try {
    ...
    if (!r.ok) return { ...empty, error: `Proxy error ${r.status}` }
    const data = await r.json()
    return { ...empty, ...data }
  } catch (e) {
    // Proxy not deployed or network error — return empty with error
    return { ...empty, error: 'SSL check unavailable: ' + (e.message || 'timeout') }
  }
}
