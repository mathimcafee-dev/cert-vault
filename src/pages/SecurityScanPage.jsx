...
// SSL CHECK — proxy handles SSL Labs + crt.sh server-side (no rate limits, no CORS)
async function checkSSLLabs(domain) {
  const empty = {
    grade: null, subject: null, issuer: null, expires: null, daysLeft: null,
    protocols: [], vulns: [], hsts: false, hstsAge: null, ipAddress: null,
    source: null, error: null, crtEntries: [], httpsDetected: false,
  }

  // Primary: our Vercel API proxy — runs server-side, no CORS, no rate limits
  try {
    const r = await fetch(`/api/ssl-check?domain=${encodeURIComponent(domain)}`, {
      signal: AbortSignal.timeout(25000),
    })
    if (r.ok) {
      const data = await r.json()
      // Proxy returned valid data with cert details
      if (data && !data.error && (data.grade || data.issuer)) {
        return { ...empty, ...data }
      }
      // Proxy returned an error but is reachable — still use what we got
      if (data && data.error) {
        empty.error = data.error
      }
    }
  } catch (_) {
    // Proxy not reachable (first deploy, local dev) — fall through
  }

  // Fallback: HTTPS DNS record proves SSL exists instantly (<300ms)
  try {
    const r = await fetch(
  ...
  return { ...empty, error: 'Could not verify SSL. Ensure api/ssl-check.js is deployed.' }
}
...
function computeScore(ssl, dmarc, spf, dkim, dns, bimi, observatory) {
  ...
  items.push({ cat:'SSL/TLS', pts:sslPts, max:30, icon:'🔒',
    detail: ssl.grade === 'Valid' ? (ssl.daysLeft != null ? `Valid · ${ssl.daysLeft}d remaining` : 'Valid certificate') : ssl.grade ? `SSL Labs: ${ssl.grade}` : ssl.error || 'No certificate found',
    ...
    <div style={{ background:'#fff', borderRadius:14, border:'1.5px solid #cffafe', padding:'22px 24px', marginBottom:14, boxShadow:'0 4px 20px rgba(8,145,178,.08)' }}>
      <div style={{ display:'flex', gap:20, flexWrap:'wrap', alignItems:'flex-start' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, flexShrink:0 }}>
          {[
            { label:'SSL',   ok: !!r.ssl.grade && !['F','T','M'].includes(r.ssl.grade), val: r.ssl.grade === 'Valid' ? (r.ssl.daysLeft != null ? `Valid (${r.ssl.daysLeft}d)` : 'Valid ✓') : r.ssl.grade || 'N/A' },
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        <Card title="SSL Summary" icon="🔒">
          <Row label="SSL Grade / Status" val={r.ssl.grade==='Valid'?(r.ssl.daysLeft!=null?`Valid ✓ · ${r.ssl.daysLeft}d remaining`:'Valid ✓'):r.ssl.grade||'N/A'} status={r.ssl.grade&&!['F','T','M'].includes(r.ssl.grade)?'great':'fail'}/>
        ...
        <Card title="SSL Labs Analysis" icon="🔒">
          <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:16, padding:'16px', background:gradeBg, borderRadius:10, border:`1.5px solid ${gradeColor}44` }}>
            <div style={{ width:72, height:72, borderRadius:'50%', background:'#fff', border:`3px solid ${gradeColor}`, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              ...
              <div style={{ fontSize:10, color:gradeColor, fontWeight:600 }}>SSL</div>
            </div>
            <div>
              <div style={{ fontSize:14, fontWeight:700, color:'#083344', marginBottom:4 }}>
                {ssl.grade==='A+'?'Exceptional — best possible configuration':ssl.grade==='A'?'Excellent — strong configuration':ssl.grade==='Valid'?(ssl.daysLeft!=null?`Certificate valid — ${ssl.daysLeft} days remaining`:'Certificate valid'):ssl.grade==='B'?'Good — minor issues to fix':ssl.grade==='C'?'Fair — significant issues present':'Poor — immediate action needed'}
              </div>
              <div style={{ fontSize:12.5, color:'#67c5d4' }}>{ssl.source || 'Certificate Transparency (crt.sh)'}</div>
