...
// SSL Labs API — industry gold standard, CORS-enabled, free
async function checkSSLLabs(domain) {
  const base = 'https://api.ssllabs.com/api/v3'
  const result = {
    grade: null, gradeTrust: null, subject: null, issuer: null,
    expires: null, daysLeft: null, protocols: [], vulns: [],
    certChain: [], keySize: null, keyAlg: null,
    hsts: false, hstsAge: null, status: 'unknown', error: null,
    fromCache: false, ipAddress: null,
  }
  try {
    // Start / get cached analysis
    const startUrl = `${base}/analyze?host=${encodeURIComponent(domain)}&fromCache=on&maxAge=24&all=done`
    const r1 = await fetch(startUrl, { signal: AbortSignal.timeout(10000) })
    if (!r1.ok) { result.error = `SSL Labs HTTP ${r1.status}`; return result }
    const data = await r1.json()

    if (data.errors?.length) { result.error = data.errors[0].message; return result }
    ...
    if (data.status === 'READY' && data.endpoints?.length) {
      ...
      result.gradeTrust = ep.gradeTrustIgnored || null
      result.ipAddress  = ep.ipAddress || null
      result.status     = ep.statusMessage || 'Ready'

      if (ep.details) {
        const d = ep.details
        // Certificate
        if (d.certChains?.length) {
          const chain = d.certChains[0]
          const leaf  = chain.certs?.[0]
          if (leaf) {
            result.subject   = leaf.subject || domain
            result.issuer    = leaf.issuerLabel || leaf.issues?.join(', ') || '—'
            ...
            if (leaf.notAfter) {
              ...
              result.daysLeft = Math.ceil((exp - new Date()) / 86400000)
            }
            result.certChain = (chain.certs || []).map(c => ({
              subject: c.subject, issuer: c.issuerLabel, keySize: c.keySize,
            }))
          }
        }
        // Protocols
        result.protocols = (d.protocols || []).map(p => ({
        ...
        const vulnChecks = [
          ...
          { key:'bleichenbacher',label:'ROBOT'            },
        ]
        result.vulns = vulnChecks
          .filter(v => d[v.key] === true || d[v.key] === 2)
          .map(v => v.label)
        // HSTS
      ...
      // Not cached — trigger and poll once more after 8s
      await new Promise(r => setTimeout(r, 8000))
      const r2 = await fetch(`${base}/analyze?host=${encodeURIComponent(domain)}&fromCache=on&maxAge=24&all=done`, { signal: AbortSignal.timeout(12000) })
      if (r2.ok) {
        const d2 = await r2.json()
        if (d2.status === 'READY' && d2.endpoints?.length) {
          result.grade  = d2.endpoints[0].grade
          result.status = 'READY'
          result.fromCache = true
        } else {
          result.error = 'SSL Labs scan in progress — try again in 60 seconds'
          result.status = d2.status
        }
      }
    }
  } catch(e) {
    result.error = e.name === 'TimeoutError' ? 'SSL Labs timeout — try again' : e.message
  }
  return result
}
...
function computeScore(ssl, dmarc, spf, dkim, dns, bimi, observatory) {
  ...
  const sslPts = ssl.grade ? (sslGradeMap[ssl.grade] ?? 0) : ssl.daysLeft > 0 ? 10 : 0
  total += sslPts
  items.push({ cat:'SSL/TLS', pts:sslPts, max:30, icon:'🔒',
    detail: ssl.grade ? `SSL Labs grade: ${ssl.grade}` : ssl.error ? `Error: ${ssl.error}` : 'No certificate found',
    status: sslPts === 30 ? 'great' : sslPts >= 18 ? 'good' : sslPts > 0 ? 'warn' : 'fail',
  })
...
async function runFullScan(domain, onProgress) {
  ...
  onProgress(15, 'Checking SSL certificate via SSL Labs…')
    ...
    <div style={{ background:'#fff', borderRadius:14, border:'1.5px solid #cffafe', padding:'22px 24px', marginBottom:14, boxShadow:'0 4px 20px rgba(8,145,178,.08)' }}>
      <div style={{ display:'flex', gap:20, flexWrap:'wrap', alignItems:'flex-start' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, flexShrink:0 }}>
          {[
            { label:'SSL',   ok: !!r.ssl.grade && !['F','T','M'].includes(r.ssl.grade), val: r.ssl.grade || 'N/A' },
...
function OverviewTab({ r }) {
  const issues = []
  // SSL
  if (!r.ssl.grade || ['F','T','M'].includes(r.ssl.grade)) issues.push({ sev:'critical', cat:'SSL', msg:`SSL grade ${r.ssl.grade||'unknown'} — certificate issue detected`, fix:'Check SSL Labs report for full details. Common issues: expired cert, weak cipher, chain errors.' })
  else if (['C','D','E'].includes(r.ssl.grade)) issues.push({ sev:'warning', cat:'SSL', msg:`SSL grade ${r.ssl.grade} — needs improvement`, fix:'Disable TLS 1.0/1.1, weak ciphers, and ensure HSTS is enabled.' })
  if (r.ssl.daysLeft !== null && r.ssl.daysLeft <= 30) issues.push({ sev:'critical', cat:'SSL', msg:`SSL certificate expires in ${r.ssl.daysLeft} days`, fix:'Renew your SSL certificate immediately to avoid browser warnings.' })
  // DMARC
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        <Card title="SSL Summary" icon="🔒">
          <Row label="SSL Labs Grade"  val={r.ssl.grade||'N/A'}   status={r.ssl.grade&&!['F','T','M'].includes(r.ssl.grade)?r.ssl.grade==='A+'||r.ssl.grade==='A'?'great':'good':'fail'}/>
          <Row label="Issuer"          val={r.ssl.issuer||'—'}     plain/>
          <Row label="Expires"         val={r.ssl.expires||'—'}    plain/>
          <Row label="Days Remaining"  val={r.ssl.daysLeft!=null?`${r.ssl.daysLeft}d`:'—'} status={!r.ssl.daysLeft?'fail':r.ssl.daysLeft>30?'great':r.ssl.daysLeft>0?'warn':'fail'}/>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:16, padding:'16px', background:gradeBg, borderRadius:10, border:`1.5px solid ${gradeColor}44` }}>
            ...
            <div>
              <div style={{ fontSize:14, fontWeight:700, color:'#083344', marginBottom:4 }}>
                {ssl.grade==='A+'?'Exceptional — best possible configuration':ssl.grade==='A'?'Excellent — strong configuration':ssl.grade==='B'?'Good — minor issues to fix':ssl.grade==='C'?'Fair — significant issues present':'Poor — immediate action needed'}
              </div>
