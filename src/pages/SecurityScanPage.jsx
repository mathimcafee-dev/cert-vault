...
// SSL CHECK — crt.sh Certificate Transparency (CORS-safe, no proxy needed)
// crt.sh has Access-Control-Allow-Origin: * — works directly from browser
async function checkSSLLabs(domain) {
  const empty = {
    ...
    ipAddress:null, fromCache:false, source:'Certificate Transparency (crt.sh)',
    error:null, crtEntries:[],
  }
  try {
    // Query crt.sh — returns ALL certs ever issued for this domain
    const url = `https://crt.sh/?q=${encodeURIComponent(domain)}&output=json`
    const r = await fetch(url, { signal: AbortSignal.timeout(12000) })
    if (!r.ok) return { ...empty, error: `crt.sh returned ${r.status}` }

    const raw = await r.json()
    if (!Array.isArray(raw) || raw.length === 0) {
      return { ...empty, error: 'No certificates found in CT logs. Domain may not have SSL.' }
    }

    const now = new Date()

    // Build entries for the CT log table
    const allSorted = [...raw].sort((a,b) => new Date(b.not_before) - new Date(a.not_before))
    const entries = allSorted.slice(0, 10).map(c => {
      const exp = new Date(c.not_after)
      const dl  = Math.ceil((exp - now) / 86400000)
      return {
        issuer:  extractOrg(c.issuer_name),
        subject: c.common_name || domain,
        from:    fmt(new Date(c.not_before)),
        to:      fmt(exp),
        daysLeft: dl,
      }
    })

    // Find the best valid cert for this exact domain
    const matchesDomain = c => {
      const cn = (c.common_name || '').toLowerCase()
      const d  = domain.toLowerCase()
      if (cn === d || cn === `*.${d}`) return true
      // name_value contains SANs separated by newlines
      return (c.name_value || '').toLowerCase().split('\n')
        .some(n => n.trim() === d || n.trim() === `*.${d}`)
    }

    const validAll  = allSorted.filter(c => new Date(c.not_after) > now)
    const validMatch = validAll.filter(matchesDomain)
    const best = validMatch[0] || validAll[0] || allSorted[0]

    if (!best) return { ...empty, crtEntries: entries, error: 'All certificates in CT logs are expired.' }

    const exp     = new Date(best.not_after)
    const daysLeft = Math.ceil((exp - now) / 86400000)
    const issuer  = extractOrg(best.issuer_name)

    return {
      ...empty,
      grade:    daysLeft > 0 ? 'Valid' : 'Expired',
      subject:  best.common_name || domain,
      issuer,
      expires:  fmt(exp),
      daysLeft: daysLeft > 0 ? daysLeft : 0,
      crtEntries: entries,
      error:    daysLeft <= 0 ? 'Certificate has expired' : null,
    }
  } catch (e) {
    const msg = e.name === 'AbortError' ? 'crt.sh request timed out — try again' : e.message
    return { ...empty, error: msg }
  }
}

function extractOrg(issuerName) {
  if (!issuerName) return '—'
  // Try O= first (organisation), then CN=
  const org = issuerName.match(/[,/]?O=([^,/]+)/)?.[1]?.trim()
  if (org && !org.startsWith('www.')) return org
  return issuerName.match(/CN=([^,/]+)/)?.[1]?.trim() || issuerName.slice(0,40)
}
