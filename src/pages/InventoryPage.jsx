...
// Robust date parser — handles DD-MM-YYYY, DD Mon YYYY, Mon DD YYYY, YYYY-MM-DD, DD-Mon-YYYY
...
// Robust CSV parser — handles quoted fields, commas inside quotes, varied line endings
...
function getAlert(row) {
  ...
  // Only skip truly inactive orders
  if (['expired','cancelled','revoked'].includes(status)) return null
  ...
  const today = new Date(); today.setHours(0,0,0,0)
  // Only skip if cert expired more than 7 days ago (grace period)
...
export default function InventoryPage({ user }) {
  ...
  async function processFile(file) {
    ...
    try {
      ...
      if (rows.length === 0) {
        alert('No rows found in file. Make sure you are uploading a valid CSV export.')
        setUploading(false)
        return
      }

      // Detect column names flexibly — TSS exports vary
      const sample = rows[0]
      const keys = Object.keys(sample)
      const find = (...candidates) => keys.find(k => candidates.some(c => k.toLowerCase().trim() === c.toLowerCase())) || ''

      const COL = {
        orderId:     find('Order ID', 'OrderID', 'Order Id', 'order id'),
        domain:      find('DomainName', 'Domain Name', 'Domain', 'CommonName', 'Common Name'),
        product:     find('ProductName', 'Product Name', 'Product', 'Certificate Type', 'CertType'),
        status:      find('Order Status', 'OrderStatus', 'Status'),
        orderDate:   find('OrderDate', 'Order Date', 'Purchase Date', 'Created Date'),
        certExpiry:  find('CertificateExpiresOn', 'Certificate Expiry', 'Cert Expiry', 'CertExpiry', 'Expiry Date', 'ValidTo', 'Valid To', 'ExpiryDate'),
        orderExpiry: find('Order Expiration Date', 'OrderExpirationDate', 'Order Expiry', 'Expiration Date'),
      }

      console.log('Detected columns:', COL)
      console.log('Sample row:', sample)

      // Re-map rows using detected columns
      const mapped = rows.map(r => ({
        'Order ID':                   r[COL.orderId]    || '',
        'DomainName':                 r[COL.domain]     || '',
        'ProductName':                r[COL.product]    || '',
        'Order Status':               r[COL.status]     || 'Active',
        'OrderDate':                  r[COL.orderDate]  || '',
        'CertificateExpiresOn':       r[COL.certExpiry] || '',
        'Order Expiration Date':      r[COL.orderExpiry]|| '',
      }))
      ...
      if (processed.length === 0) {
        alert(`Found ${rows.length} rows but none passed the filter.

Common causes:
- Status column shows 'Expired' or 'Cancelled' (these are excluded)
- Certificate expiry date column not detected (found: "${COL.certExpiry || 'NOT FOUND'}")
- Domain column not detected (found: "${COL.domain || 'NOT FOUND'}")
...
${keys.join(', ')}`)
      ...
      for (let i = 0; i < ins.length; i += 50) {
        await supabase.from('certificate_inventory').insert(ins.slice(i, i+50))
      }
      await loadFromDb()
