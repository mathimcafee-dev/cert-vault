...
const COMING_SOON_IDS = ['ssl-checker']
...
export default function App() {
  ...
  const navigate = (p) => {
    // Handle cert-labs sub-tools: 'cert-labs:csr-decoder' etc
    if (p.startsWith('cert-labs:')) {
      setCertLabsTool(p.split(':')[1])
      setPage('cert-labs')
      return
    }
    // Handle dmarc sub-tabs: 'dmarc:bulk', 'dmarc:spf-gen' etc
    if (p.startsWith('dmarc:')) {
      setDmarcTab(p.split(':')[1])
      setPage('dmarc')
      return
    }
    // Handle dns sub-tabs: 'dns:health', 'dns:blacklist' etc
    if (p.startsWith('dns:')) {
      setDnsTab(p.split(':')[1])
      setPage('dns')
      return
